...
class GenericNode {
 public:
  ...
  JsonPathResult<const NodeType> AtJsonPath(const StringView jsonpath) const {
    return AtJsonPathCommon(downCast(), jsonpath);
  }

 private:
  template <typename DerivedPtr>
  static sonic_force_inline JsonPathResult<std::remove_pointer_t<DerivedPtr>>
  AtJsonPathCommon(DerivedPtr self, const StringView jsonpath) {
    using ResultNodeType = std::remove_pointer_t<DerivedPtr>;
    JsonPathResult<ResultNodeType> ret = {};
    ...
    if (!path.Parse(StringView(pathpadd.data(), jsonpath.size()))) {
